...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	const [timeline, notes, events, employees] = await Promise.all([
		...
		db.select().from(note)
			.where(and(eq(note.ownerType, 'workstation'), eq(note.ownerId, params.id)))
...
export const actions: Actions = {
	assign: async ({ request, params, locals }) => {
		...
		if (isReassign && prevEmp) {
			...
			await audit.log({ action: 'workstation.assigned', organizationId: orgId, subjectType: 'employee', subjectId: emp.id, subjectLabel: `${emp.firstName} ${emp.lastName}`, actorId: locals.user!.id, actorLabel: locals.user!.name, metadata: { related: { type: 'workstation', id: ws.id, label: `${ws.make} ${ws.model} (${ws.serialNumber})` } } });
		} else {
			await audit.logBoth({
