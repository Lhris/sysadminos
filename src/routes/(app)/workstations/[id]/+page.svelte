<script lang="ts">
	...
	const sb  = $derived(statusBadge[ws.status as Status] ?? { label: ws.status, cls: 'border-border bg-muted text-muted-foreground' });
...
<main class="flex min-w-0 flex-1 flex-col gap-6 overflow-y-auto px-10 py-9">
		...
		<Sheet.Root bind:open={sheetOpen}>
			...
			<Sheet.Content class="flex flex-col gap-0 p-0 sm:max-w-[480px]">
				<Sheet.Header class="border-b px-6 py-4">
					<Sheet.Title class="text-[15px] font-bold tracking-[-0.02em]">Edit Workstation</Sheet.Title>
				</Sheet.Header>
	...
	<div class="grid grid-cols-[1fr_360px] items-start gap-4">
		...
		<div class="flex flex-col gap-4">
			...
			<Card.Root>
				<Card.Header>
					<Card.Title class="text-[15px] font-bold tracking-[-0.02em]">Details</Card.Title>
				</Card.Header>
				<Card.Content class="flex flex-col gap-0">
			...
			<Card.Root class="overflow-hidden">
				<Card.Header class="flex flex-row items-center justify-between pb-3">
					<Card.Title class="text-[15px] font-bold tracking-[-0.02em]">Assignment</Card.Title>
					...
					{#if ws.assignedEmployeeId && ws.assignedEmployeeFirstName}
						<div class="flex items-center justify-between px-5 py-[11px]">
							<div class="flex flex-col gap-[3px]">
								<a href="/employees/{ws.assignedEmployeeId}" class="text-[13.5px] font-medium text-foreground no-underline hover:underline">
			...
			<Card.Root class="overflow-hidden">
				<Card.Header class="flex flex-row items-center justify-between pb-3">
					<Card.Title class="text-[15px] font-bold tracking-[-0.02em]">Assignment History</Card.Title>
					<button type="button" onclick={() => { showAddEntry = !showAddEntry; editingEntryId = null; }} class="text-[12px] font-medium text-muted-foreground transition-colors hover:text-foreground">
							...
							{#if editingEntryId === entry.id}
								...
								>
									...
									<div class="flex items-center gap-2">
										...
										<button type="button" onclick={() => (editingEntryId = null)} class="text-[12px] text-muted-foreground hover:text-foreground">×</button>
									</div>
								</form>
							{:else}
								<div class="grid grid-cols-[90px_90px_1fr_1fr_80px] items-center gap-2 border-b border-border/60 px-4 py-2 last:border-b-0">
									...
									<div class="flex items-center gap-2">
										<button type="button" onclick={() => { editingEntryId = entry.id; showAddEntry = false; }} class="text-[12px] text-muted-foreground transition-colors hover:text-foreground">Edit</button>
		...
		<div class="flex flex-col gap-4">
			...
			<Card.Root class="overflow-hidden">
				<Card.Header class="pb-3">
					<Card.Title class="text-[15px] font-bold tracking-[-0.02em]">Notes</Card.Title>
				</Card.Header>
				<Card.Content class="flex flex-col gap-0 p-0">
