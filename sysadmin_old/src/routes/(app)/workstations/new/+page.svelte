...
<main class="flex min-w-0 flex-1 flex-col gap-6 overflow-y-auto px-10 py-9">
	...
	<form method="POST" use:enhance class="flex max-w-2xl flex-col gap-5">
			...
			<Card.Content class="flex flex-col gap-4">
				...
				<div class="flex flex-col gap-1.5">
					...
					<Input
						...
						placeholder="C02XY1234AB"
						value={values.serialNumber ?? ''}
						class="font-mono {errors.serialNumber ? 'border-destructive' : ''}"
					/>
					{#if errors.serialNumber}
						<p class="text-[12px] text-destructive">{errors.serialNumber}</p>
					{/if}
				</div>
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="flex flex-col gap-1.5">
						...
						<Input
							id="make" name="make" placeholder="Apple"
							value={values.make ?? ''}
							class={errors.make ? 'border-destructive' : ''}
						/>
						{#if errors.make}
							<p class="text-[12px] text-destructive">{errors.make}</p>
						{/if}
					</div>
					<div class="flex flex-col gap-1.5">
						<Label for="model" class="text-[13px]">Model</Label>
						<Input
							id="model" name="model" placeholder='MacBook Pro 14"'
							value={values.model ?? ''}
							class={errors.model ? 'border-destructive' : ''}
						/>
						{#if errors.model}
							<p class="text-[12px] text-destructive">{errors.model}</p>
						{/if}
					</div>
				</div>
				...
				<div class="flex flex-col gap-1.5">
					...
					<select
						...
						class="{selectClass} {errors.deviceType ? 'border-destructive' : ''}"
					>
					...
					{#if errors.deviceType}
						<p class="text-[12px] text-destructive">{errors.deviceType}</p>
					{/if}
				</div>
		...
		<Card.Root>
			<Card.Header>
				<Card.Title class="font-heading text-[15px] font-bold tracking-[-0.02em]">Status</Card.Title>
			</Card.Header>
			<Card.Content>
